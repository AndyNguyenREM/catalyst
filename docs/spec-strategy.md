...
|----------|------------------------|-----------------|
| Use hosted LLM API for chat agent | Local vLLM, smaller on-device model | Best tool-use support, streaming quality |
...
If using AI assistants, create an instruction file in the project root. For Codex, use `AGENTS.md`; for Claude Code, use `CLAUDE.md`.
...
1. Read `AGENTS.md` or `CLAUDE.md` (if AI) or `README.md` (if human)
